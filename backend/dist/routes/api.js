...
router.post("/ai/update-risk", async (req, res) => {
    ...
    try {
        ...
        if (allBoreholes.length > 0) {
            const borehole = allBoreholes[Math.floor(Math.random() * allBoreholes.length)];
            const change = Math.random() * 10 - 5; // +/- 5%
            let newLevel = Math.max(0, Math.min(100, (borehole.water_level || 100) + change));
            await prisma.borehole.update({
                where: { id: borehole.id },
                data: { water_level: newLevel },
            });
            aiSummary += ` Adjusted ${borehole.name} water level to ${newLevel.toFixed(1)}%.`;
        }
